...
datacleanr_server <- function(input, output, session, dataset, df_name, is_on_disk) {
  ...
  action_tracking <- shiny::reactiveValues(
    plot_start = NULL,
    controls = NULL
  )
  ...
  max_id_original_traces <- shiny::reactive({
    dplyr::n_groups(datareactive())
  })
  ...
  output$gvar <- shiny::reactive({
    gvar()
  })
  # check-box for grouping
  grouping_check <- shiny::callModule(
    module = module_server_checkbox,
    "grouptick",
  ...
  gobutton_reactive <- shiny::reactive({
    input$gobutton
  })
  ...
  shiny::observe({
    ...
    filt_out <- shiny::callModule(
      module = module_server_df_filter,
      id = "check",
  ...
  shiny::observeEvent(
    ...
    },
    {
      ...
      selected_data$df <- handle_sel_outliers(
        sel_old_df = selected_data$df,
        sel_new = selected
      )
    }
  )
  ...
  shiny::observeEvent(
    ...
    },
    {
      ...
      is_spatial_plot <- identical(
        c(
          as.character(selector_vals$xvar),
  ...
  output$histogramupdatebutton <- shiny::renderUI({
    ...
    shiny::tagList(shiny::actionButton(
      inputId = "histogramupdate",
      label = "Update",
  ...
  shiny::observeEvent(input$`config-save`, {
    ...
    saveRDS(
      object = out_data()$dcr_df,
      file = out_path()$file_out_cleaned
    )
    saveRDS(
      object = list(
        dcr_condition_df = out_data()$dcr_condition_df,
    ...
    writeLines(
      text = enc2utf8(out_data()$dcr_code),
  ...
  text_filtering_side_panel <- shiny::tagList(
    ...
    shiny::tags$p(
      "For example, valid statements for filtering",
      shiny::tags$b("iris"),
      "are:"
    ),
    shiny::tags$ol(
  ...
  text_annotate_side_panel <- shiny::tagList(
    ...
    shiny::p(
      "Labels are collected and provided as an additional column",
  ...
  text_plot_main_panel <- shiny::tagList(
    ...
    shiny::p(
      "The legend entries correspond with the row-numbers (i.e. groups) of the",
      shiny::tags$b("Data Overview"),
      "Table.",
